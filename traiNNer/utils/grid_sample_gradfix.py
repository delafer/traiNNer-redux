...
def grid_sample(input, grid, mode=None, padding_mode=None, align_corners=None):
    """Compatibility wrapper for grid_sample that handles additional parameters."""
    ...
    return torch.nn.functional.grid_sample(
        input=input, grid=grid, padding_mode=padding_mode, align_corners=align_corners
    )
...
class _GridSample2dForward(torch.autograd.Function):
    ...
    def forward(ctx, input, grid):
        ...
        output = torch.nn.functional.grid_sample(
            input=input, grid=grid, padding_mode="zeros", align_corners=False
        )
