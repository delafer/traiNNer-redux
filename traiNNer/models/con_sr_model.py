...
class SRModel(BaseModel):
    ...
    def __init__(self, opt: ReduxOptions) -> None:
        ...
        # === NEG-EMA: статические параметры для контрастной негативной потери ===
        # НИЧЕГО не читаем из конфигов, чтобы не ломать сериализацию.
        # === NEG-EMA: статические параметры контрастных EMA ===
        # НИЧЕГО не читаем из конфига; правь список интервалов тут.
        self.NEG_CONTRAST_ENABLED: bool = True

        # Список интервалов (в итерациях) для независимых EMA-негативов.
        # Пример: три «частоты» обновления.
        self.NEG_EMA_SCHEDULE_ITERS: list[int] = [100, 200, 500, 1000]

        # Бета сглаживания для всех негативных EMA.
        self.NEG_EMA_BETA: float = 0.999

        # Контейнеры для EMA-негативов
        self._neg_emas: list[EMA] = []  # список EMA-врапперов
        # (вместо снимков state_dict/буфера tmp-модели ничего больше не нужно)
    ...
    def _ensure_neg_tmp_model(self) -> None:
        """Создаёт (лениво) вспомогательную модель для прогонов бэкапов EMA."""
        if self._neg_tmp_model is None:
            assert self.opt.network_g is not None
            tmp = build_network({**self.opt.network_g, "scale": self.opt.scale})
            tmp = self.model_to_device(tmp)
            tmp.eval()
            for p in tmp.parameters():
                p.requires_grad = False
            self._neg_tmp_model = tmp

    @torch.no_grad()
    def _forward_neg_ema_rgb(self, lq_pixfmt: torch.Tensor, ema: EMA) -> torch.Tensor:
        with torch.autocast(
            device_type=self.device.type, dtype=self.amp_dtype, enabled=self.use_amp
        ):
            out = ema.ema_model(lq_pixfmt)  # если у твоей сети есть флаг neg
        if isinstance(out, tuple | list):
            out = out[0]
        rgb = pixelformat2rgb_pt(out, self.gt, self.opt.output_pixel_format)
        return rgb

    def _maybe_capture_ema_backup(self, current_iter: int) -> None:
        """Сохранить EMA-бэкап по расписанию с равным шагом."""
        if not self.NEG_CONTRAST_ENABLED:
            return
        if self.net_g_ema is None:
            return
        if current_iter < self._next_backup_iter:
            return

        logger = get_root_logger()
        # забираем state_dict EMA-модели (на CPU, без градиентов)
        assert isinstance(self.net_g_ema.ema_model, nn.Module)
        ema_sd = {
            k: v.detach().cpu().clone()
            for k, v in self.net_g_ema.ema_model.state_dict().items()
        }
        self._neg_backups.append((current_iter, ema_sd))
        if len(self._neg_backups) > self.NEG_MAX_BACKUPS:
            self._neg_backups.pop(0)

        logger.info(
            "Saved EMA backup for contrastive negatives @ iter %d (total %d / max %d).",
            current_iter,
            len(self._neg_backups),
            self.NEG_MAX_BACKUPS,
        )
        self._next_backup_iter += self.NEG_BACKUP_GAP_ITERS

    def _maybe_update_neg_emas(self, current_iter: int) -> None:
        """Раз в заданный интервал обновляем каждую NEG-EMA сглаживанием (без копирования весов)."""
        if not (self.NEG_CONTRAST_ENABLED and self._neg_emas):
            return
        # не обновляем, если шаг оптимизатора был «пропущен» из-за AMP (как для обычной EMA)
        skip = self.use_amp and self.optimizers_skipped and self.optimizers_skipped[0]
        if skip:
            return
        for gap, ema in zip(self.NEG_EMA_SCHEDULE_ITERS, self._neg_emas, strict=False):
            if gap > 0 and (current_iter % gap == 0):
                ema.update()  # гладкое EMA-обновление в этот момент
    ...
    def init_training_settings(self) -> None:
        ...
        # === ИНИЦИАЛИЗАЦИЯ НЕГАТИВНЫХ EMA ПО РАСПИСАНИЮ ===
        # Каждая NEG-EMA "смотрит" на те же параметры net_g, но обновляем её только по своему интервалу.
        self._neg_emas = []
        if self.NEG_CONTRAST_ENABLED and self.NEG_EMA_SCHEDULE_ITERS:
            for _gap in self.NEG_EMA_SCHEDULE_ITERS:
                ema_neg = EMA(
                    self.get_bare_model(self.net_g),
                    ema_model=None,  # пусть EMA сама создаст копию
                    beta=self.NEG_EMA_BETA,
                    allow_different_devices=True,
                    update_after_step=0,
                    update_every=1,
                    power=1.0,
                    update_model_with_ema_every=None,  # управляем обновлением вручную
                ).to(device=self.device, memory_format=self.memory_format)
                ema_neg.step = ema_neg.step.to(device=torch.device("cpu"))
                self._neg_emas.append(ema_neg)
    ...
    def optimize_parameters(
        ...
        with torch.autocast(
            ...
            if self.optimizer_g is not None:
                ...
                # mini = pixelformat2rgb_pt(mini, self.gt, self.opt.output_pixel_format)
                assert isinstance(self.output, Tensor)
                l_g_total = torch.tensor(0.0, device=self.output.device)

                lq_target = None
                # print(current_iter,current_accum_iter)
                # === БЛОК ВЫЧИСЛЕНИЯ ЛОССОВ (добавлен l_g_conspark) ===
                for label, loss in self.losses.items():
                    target = self.gt

                    # Спец. ветка: контрастная перцептивная потеря Conspark
                    if label == "l_g_conspark":
                        # if current_iter<5000:
                        #     continue
                        # собрать SR-негативы из EMA-бэкапов
                        neg_sr_list: list[Tensor] = []
                        if self.NEG_CONTRAST_ENABLED and self._neg_emas:
                            # with torch.inference_mode():
                                for ema in self._neg_emas:
                                    neg_sr = self._forward_neg_ema_rgb(lq, ema)
                                    neg_sr_list.append(neg_sr)

                        # собрать HR-негативы роллами по батчу
                        neg_hr_list: list[Tensor] = []

                        # вызвать лосс: он сам делает контраст с переданными негативами
                        l_g_loss = loss(self.output, target, neg_sr_list, neg_hr_list)
                        ...
                        continue  # к следующему лоссу

                    # --- старая ветка для отрицательных весов (bicubic-fallback) ---
                    ...
                    # elif label == "l_g_hsluv":
                    #     b, c, h, w = mini.shape
                    #     l_g_loss = loss(
                    #         mini,
                    #         F.interpolate(
                    #             target, size=(h, w), mode="bilinear", align_corners=True
                    #         ),
                    #     )
                    else:
                ...
                # === /БЛОК ВЫЧИСЛЕНИЯ ЛОССОВ ===

                if not l_g_total.isfinite():
                    raise RuntimeError(
                        "Training failed: NaN/Inf found in loss. Try reducing the learning rate. "
                        "If training still fails, please file an issue: https://github.com/the-database/traiNNer-redux/issues"
                    )
        ...
        # обновить EMA и при необходимости зафиксировать бэкап
        if self.net_g_ema is not None and apply_gradient:
            if not (self.use_amp and self.optimizers_skipped[0]):
                self.net_g_ema.update()
            # вместо сохранения снимка — выборочное EMA-обновление негативных EMA
        self._maybe_update_neg_emas(current_iter)
