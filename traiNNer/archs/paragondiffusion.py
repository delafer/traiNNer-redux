...
class DownBlock(nn.Module):
    def __init__(
        self, in_channels, out_channels, num_layers, time_emb_dim, use_checkpoint
    ) -> None:
        super().__init__()
    ...
    def forward(self, x, time_emb):
        ...
        for resnet in self.resnets:
            if self.use_checkpoint:
                x = checkpoint(resnet, x, time_emb)
            else:
                x = resnet(x, time_emb)
...
class ParagonDiffusion(nn.Module):
    ...
    def __init__(
        ...
        print(
            f"Initializing ParagonDiffusion with {channels} base channels, block structure {num_blocks}, and checkpointing={use_checkpoint}..."
        )
        self.use_checkpoint = use_checkpoint
        self.vae: AutoencoderKL = AutoencoderKL.from_pretrained(vae_model_name)
        ...
        for i, num_layers in enumerate(num_blocks):
            ...
            self.down_blocks.append(
                DownBlock(in_ch, out_ch, num_layers, time_emb_dim, use_checkpoint)
            )
            in_ch = out_ch

        self.mid_block1 = ResnetBlock(in_ch, in_ch, time_emb_dim)
        self.mid_block2 = ResnetBlock(in_ch, in_ch, time_emb_dim)
        ...
        for i, num_layers in reversed(list(enumerate(num_blocks))):
            ...
            self.up_blocks.append(
                UpBlock(in_ch, out_ch, num_layers, time_emb_dim, use_checkpoint)
            )
            in_ch = out_ch
    ...
    def forward(self, gt_image: torch.Tensor, lr_image: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            z1 = self.vae.encode(gt_image).latent_dist.mean  # type: ignore
        z1 = z1 * self.vae.config.scaling_factor  # type: ignore
    ...
    def predict_flow(
        ...
        if self.use_checkpoint:
            x = checkpoint(self.mid_block1, x, time_emb)
            x = checkpoint(self.mid_block2, x, time_emb)
        else:
    ...
    def sample(self, lr_image: torch.Tensor, num_steps: int = 10) -> torch.Tensor:
        ...
        z1 = zt / self.vae.config.scaling_factor  # type: ignore
        img = self.vae.decode(z1).sample  # type: ignore
        return img
