...
def get_loss_schedules(total_iter: int = 140000) -> dict[str, np.ndarray]:
    ...
    for loss in losses:
        weight_array = calculate_scheduled_weight(
            int(loss["start_iter"]),
            float(loss["start_weight"]),
            int(loss["target_iter"]),
            float(loss["target_weight"]),
            total_iter,
        )

        # Handle LDL disable_after
        if "disable_after" in loss:
            weight_array[int(loss["disable_after"]) :] = 0
...
def create_loss_visualization():
    ...
    fig.suptitle(
        "Loss Weight Evolution - ParagonSR2 Static-S with Feature Matching",
        fontsize=18,
        fontweight="bold",
        y=0.95,
    )
    ...
    for milestone, label in zip(milestones, milestone_labels, strict=False):
        ax0.axvline(x=milestone, color="black", linestyle="--", alpha=0.5, linewidth=1)
        ax0.text(
            milestone,
            ax0.get_ylim()[1] * 0.9,
            label,
    ...
    ax0.set_xlabel("Training Iterations", fontsize=10)
    ax0.set_ylabel("Loss Weight", fontsize=10)
    ax0.set_title("Complete Loss Schedule Overview", fontsize=12, fontweight="bold")
    ax0.grid(True, alpha=0.3)
    ax0.legend(fontsize=8, loc="upper right")
    ax0.set_xlim(0, total_iter)
    ...
    for i, category in enumerate(categories):
        ax = ax_flat[i + 1]  # Skip first subplot (overview)
        ...
        # Add milestone markers to each plot
        for milestone in milestones:
            ax.axvline(
                x=milestone, color="gray", linestyle=":", alpha=0.3, linewidth=0.5
            )
...
def analyze_schedule(schedules: dict) -> str:
    ...
    # Strategic analysis
    analysis.append("")
    analysis.append("5. STRATEGIC ANALYSIS:")

    # Count losses by category over time
    categories = {}
    for name, data in schedules.items():
    ...
    for cat_name, cat_losses in categories.items():
        # Find peak activity periods
        total_weights = np.zeros(total_iter + 1)
        for _, weights in cat_losses:
            total_weights += weights

        if np.max(total_weights) > 0:
            peak_period = np.argmax(total_weights)
            analysis.append(
                f"   {cat_name}: Peak activity around {peak_period:,} iterations"
            )
